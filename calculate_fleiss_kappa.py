...
label_mapping = {'compressed or angular discrete glands': 0,
...
label_renaming = {"poorly formed and fused glands": "4 - poorly formed glands",
...
def calculate_kappa_per_group_and_label(df: pd.DataFrame, out_path: Path, use_sub_explanations: bool):
    ...
    for group_name, group_series in group_grpd_df:
        ...
        print(f"annotators: {group_series['annotator'].unique()}")
        print(f"TMAs: {group_series['TMA_identifier'].nunique()}")
        ...
        for an_idx, annotation in enumerate(labels):
            ...
            for tma, tma_series in group_series.groupby('TMA_identifier'):
                ...
                current_group_current_annotation_existance_list.append(np.array(get_subject_description_per_rater(tma_series, annotators, annotation, use_sub_explanations)))
                if num_annotators == 3:
            ...
            print("kappa:", fl_kappa)
            print(f"CI (95%): {lower} | {upper}")
    ...
    renamed_labels = labels if use_sub_explanations==None else [base_label_mapping[lbl] for lbl in labels] if use_sub_explanations else [label_renaming[lbl] for lbl in labels]
    ...
    sns.heatmap(kappas_df, annot=True, fmt='.3g', yticklabels=renamed_labels, cmap='Blues', cbar=False, annot_kws={"size": 14}, linewidths=.5)
    ...
    np.save(out_path / f"{'expl' if use_sub_explanations==False else 'sub-expl' if use_sub_explanations == True else 'pattern'}_kappas_y-lables.npy", renamed_labels)

    figsize = (6, 2.6) if use_sub_explanations == None else (5,16) if use_sub_explanations == True else (10, 9)
    plt.figure(figsize=(figsize))
    ax_bp = sns.boxplot(data=kappas_df.T, color='#77b5d9', showfliers=False, width=0.5, native_scale=False, orient='h', showmeans=True, meanprops={"marker":"o","markerfacecolor":"white", "markeredgecolor":"gray"}) #data=kappas_df.T, palette=cmap
    plt.xlim(-0.3, 1.1)
    ...
    sns.heatmap(group_kappas_df, annot=True, yticklabels='', fmt='.3g', vmin=-0.24, vmax=1, square=True, cbar_kws={'shrink': 0.3}, cmap='Blues', cbar=False, annot_kws={"size": 20}, linewidths=.5)
    ...
    c_bar = {"shrink": 0.3}
    plt.figure(figsize=figsize)
    ax = sns.heatmap(pd.DataFrame(label_kappa).T, annot=True, yticklabels='', fmt='.3g', vmin=-0.24, vmax=1, cbar_kws=c_bar, cmap='Blues', cbar=False, annot_kws={"size": 20}, linewidths=.5, square=True)
