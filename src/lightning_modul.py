...
def initialize_torchmetrics(nn_module, num_classes, max_num_datasets=1, metrics="all"):
    ...
    if metrics == "all" or "conf_matrix" in metrics:
        ...
        nn_module.conf_matrix = nn.ModuleDict(
            {
                step: nn.ModuleList(
                    [MulticlassConfusionMatrix(num_classes=num_classes, normalize=None) for _ in range(max_num_datasets)]
                )
                for step in ["train_split", "val_split", "test_split"]
            }
        )
    ...
    if metrics == "all" or "avg_prec" in metrics:
        ...
        nn_module.avg_prec = nn.ModuleDict(
            ...
            {
                step: nn.ModuleList(
                    [MulticlassAveragePrecision(num_classes=num_classes, average="macro") for _ in range(max_num_datasets)]
                )
                for step in ["train_split", "val_split", "test_split"]
            }
        )
...
def log_metrics(l_model, split, d_idx, logits, y, losses):
    ...
    if "loss" in l_model.metrics_to_track:
        ...
        for loss_name, loss in losses.items():
            ...
            l_model.log(loss_str, loss, on_step=True if loss_name in ["loss", "KL"] else False,
                        on_epoch=True, prog_bar=True if loss_name in ["loss", "KL"] else False)
    ...
    if use_unique_max:
        ...
        if unique_max.sum() > 0:
            ...
            if "accuracy" in l_model.metrics_to_track:
                ...
                l_model.log(
                    f"{split}_acc_unique_max", l_model.accuracy[split_nn_dict][d_idx], on_step=False, on_epoch=True, prog_bar=False
                )
            ...
            if "f1_score" in l_model.metrics_to_track:
                ...
                l_model.log(
                    f"{split}_f1_unique_max", l_model.f1_score[split_nn_dict][d_idx], on_step=False, on_epoch=True, prog_bar=False
                )

            if "DICE" in l_model.metrics_to_track:
                l_model.DICE[split_nn_dict][d_idx](preds, y_one_hot)
                l_model.log(
                    f"{split}_DICE_unique_max", l_model.DICE[split_nn_dict][d_idx], on_step=False, on_epoch=True, prog_bar=False
                )

            if "b_DICE" in l_model.metrics_to_track:
                l_model.b_DICE[split_nn_dict][d_idx](preds, y_one_hot)
                l_model.log(
                    f"{split}_b_DICE_unique_max", l_model.b_DICE[split_nn_dict][d_idx], on_step=False, on_epoch=True, prog_bar=False
                )
    else:
    ...
    if "b_soft_DICE" in l_model.metrics_to_track:
        ...
        l_model.log(
            f"{split}_b_soft_DICE", l_model.b_soft_DICE[split_nn_dict][d_idx], on_step=False, on_epoch=True, prog_bar=False
        )

    if "soft_DICE" in l_model.metrics_to_track:
        l_model.soft_DICEDataset[split_nn_dict][d_idx](sm, y)
        l_model.log(
            f"{split}_soft_DICEDataset", l_model.soft_DICEDataset[split_nn_dict][d_idx], on_step=False, on_epoch=True, prog_bar=False
        )

    if "L1" in l_model.metrics_to_track:
        l_model.L1[split_nn_dict][d_idx](sm, y)
        l_model.log(
            f"{split}_L1", l_model.L1[split_nn_dict][d_idx], on_step=False, on_epoch=True, prog_bar=False
        )


LIT_SEGMENTER_LOSS_FUNCTIONS = {"CE": F.cross_entropy,
    ...
    def configure_optimizers(self):
        ...
        if patience > 0:
            lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
                optimizers, mode=self.hparams["direction"], patience=self.hparams["patience"]
            )
